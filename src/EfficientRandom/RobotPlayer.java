...
public strictfp class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        if(rc.getType() == RobotType.HEADQUARTERS) {
            ...
            rc.writeSharedArray(0, headquarters.x);
            rc.writeSharedArray(1, headquarters.y);
        }
        else headquarters = new MapLocation(rc.readSharedArray(0), rc.readSharedArray(1));
    ...
    static void runHeadquarters(RobotController rc) throws GameActionException {
        ...
        */
        //Trying prioritizing carriers. Will likely want a state machine
    ...
    static void runCarrier(RobotController rc) throws GameActionException {
        ...
        if(headquarters.isAdjacentTo(me) && weight > 12) {
            if(rc.canTransferResource(headquarters, ResourceType.ADAMANTIUM, 5))
                rc.transferResource(headquarters, ResourceType.ADAMANTIUM, rc.getResourceAmount(ResourceType.ADAMANTIUM));
            if(rc.canTransferResource(headquarters, ResourceType.MANA, 5))
                rc.transferResource(headquarters, ResourceType.MANA, rc.getResourceAmount(ResourceType.MANA));
            if(rc.canTransferResource(headquarters, ResourceType.ELIXIR, 5))
        ...
        // Also try to move randomly, or towards headquarters if full.
        int randDir = rng.nextInt(directions.length);
        Direction dir = weight == 40 ? towards(me, headquarters) : directions[randDir++%directions.length];
